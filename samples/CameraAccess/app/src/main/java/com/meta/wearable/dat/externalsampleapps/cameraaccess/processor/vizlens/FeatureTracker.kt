...
class FeatureTracker(context: Context) {
    ...
    companion object {
        ...
        private const val LOG_EVERY_N_FRAMES = 50  // Only log detailed diagnostics periodically
    }
    ...
    // Reusable buffers to reduce GC pressure in hot path
    private val pixelBuffer = IntArray(INPUT_SIZE * INPUT_SIZE)
    private val grayBuffer = FloatBuffer.allocate(INPUT_SIZE * INPUT_SIZE)
    private val matchAccumulator = FloatArray(512 * 4)  // max 512 keypoints × 4 floats each
    ...
    init {
        try {
            ...
            val sessionOptions = OrtSession.SessionOptions().apply {
                ...
                try {
                    addNnapi()  // Delegate supported ops to Android NNAPI (GPU/DSP/NPU)
     ...
     * Pre-computes and caches the ONNX tensor for the reference image.
     */
    fun setReferenceFrame(bitmap: Bitmap, textLabels: List<TextLabel>) {
        // Release previous cached tensor
        ...
        // Pre-compute and cache the reference tensor
        val env = ortEnvironment
        val ref = referenceFrame
        if (env != null && ref != null) {
            cachedRefTensor = preprocessImage(ref, env)
    ...
    fun computeHomography(currentFrame: Bitmap): FloatArray? {
        ...
        val refTensor = cachedRefTensor ?: run { Log.w(TAG, "computeHomography: cachedRefTensor is null"); return null }
        ...
        try {
            ...
            if (verbose) {
                Log.d(TAG, "computeHomography: refFrame=${referenceFrame!!.width}x${referenceFrame!!.height}, curFrame=${currentFrame.width}x${currentFrame.height}, running inference...")
            }
            ...
            val matchCount = parseMatchesInto(outputs, verbose)

            // Clean up current frame tensor only (reference tensor is cached)
            curTensor.close()
            outputs.close()
            ...
            // Compute homography using RANSAC (uses matchAccumulator[0..matchCount*4])
            val homography = computeHomographyRANSAC(matchAccumulator, matchCount)

            if (homography != null) {
                consecutiveFailures = 0
                if (verbose) Log.d(TAG, "Homography computed successfully with $matchCount match pairs")
            } else {
                consecutiveFailures++
                if (verbose) Log.w(TAG, "computeHomography: RANSAC failed")
            }
     ...
     * Reuses pre-allocated buffers to minimize GC pressure.
     */
    private fun preprocessImage(bitmap: Bitmap, env: OrtEnvironment): OnnxTensor {
        ...
        // We need a fresh FloatBuffer for each tensor (ONNX Runtime takes ownership of the data)
    ...
    /**
     * Parse matched keypoint pairs from ONNX output directly into [matchAccumulator].
     * Returns the number of valid match pairs found.
     * Avoids mutableListOf allocation in the hot path.
     */
    private fun parseMatchesInto(outputs: OrtSession.Result, verbose: Boolean): Int {
        ...
        try {
            ...
            if (verbose) {
                Log.d(TAG, "parseMatches: kpts0=${kpts0?.info?.shape?.contentToString()} (${kpts0?.info?.type}), matches0=${matchIdx?.info?.shape?.contentToString()}, mscores0=${scores?.info?.shape?.contentToString()}")
            }
     ...
     * Input: matches array [x1, y1, x2, y2, ...] and number of valid match pairs.
     */
